#+TITLE: Prometheus 存储引擎分析
#+DATE: 2021-04-11T22:42:54+08:00
#+TAGS[]: tsdb, prometheus
#+CATEGORIES[]:
Prometheus 作为云原生时代的时序数据库， 是当下最流行的监控平台之一，尽管其整体架构一直没怎么变，但其底层的存储引擎却演进了几个版本，感兴趣的读者可参考 [[https://tech.ipalfish.com/blog/2020/03/31/the-evolution-of-prometheus-storage-layer/][Prometheus 存储层的演进]]。本文主要介绍 Prometheus V2 版本（现在主要使用）的存储格式细节，以及查询如何定位到符合条件的数据，旨在通过本文的分析，对 Prometheus 的存储引擎有更深入了解。

说明：本文并不会涉及到查询的解析与函数求值过程。代码分析基于 [[https://github.com/prometheus/prometheus/tree/v2.25.2][v2.25.2]] 版本。
...
对于云原生场景来说，另一个特点是数据生命周期短，一次容器的扩缩容会导致时间线膨胀一倍。
...
#+end_src
可以看到，数据目录主要有以下几部分：
- block，一个时间段内（默认 2 小时）的所有数据，只读，用 [[https://github.com/oklog/ulid][ULID]] 命名。每一个 block 内主要包括：
...
每个部分的具体编码格式，可参考官方文档 [[https://github.com/prometheus/prometheus/blob/main/tsdb/docs/format/index.md][Index Disk Format]]，这里重点讲述一次查询是如何找到符合条件的数据的：
- 首先在 Posting Offset Table 中，找到对应 label 的 Postings 位置
#+CAPTION: offset table 到 postings 的映射过程
...
#+CAPTION: series 到 chunks 数据文件的映射过程
[[https://img.alicdn.com/imgextra/i2/581166664/O1CN01ncQkYZ1z6A3SfXvO3_!!581166664.jpg]]
...
#+end_src
  ...
  - 下面代码为根据 label 查询 postings 的逻辑，完整可见 index 的 [[https://github.com/prometheus/prometheus/blob/bda05a23ada314a0b9806a362da39b7a1a4e04c3/tsdb/index/index.go#L1555][Postings 方法]]：
   #+begin_src go
...
#+end_src
- [[https://github.com/prometheus/prometheus/blob/bda05a23ada314a0b9806a362da39b7a1a4e04c3/tsdb/head.go#L1861][stripeSeries]] 是比较的核心结构，series 字段的 key 为时间线，采用[[https://gitee.com/mirrors/prometheus/blob/bda05a23ada314a0b9806a362da39b7a1a4e04c3/tsdb/head.go#L1771][自增]]方式生成；value 为 memSeries，内部有存储具体数据的 chunk，采用分段锁思路来减少锁竞争。
** 使用方式
对于一个查询，大概涉及的步骤：
1. 根据 label 查出所涉及到的时间线，然后根据 filter 类型，进行集合运算，找出符合要求的时间线
2. 根据时间线信息与时间范围信息，去 block 内查询符合条件的数据

在第一步主要在 [[https://github.com/prometheus/prometheus/blob/bda05a23ada314a0b9806a362da39b7a1a4e04c3/tsdb/querier.go#L221][PostingsForMatchers]] 函数中完成，主要有下面几个优化点：
- 对于取反的 filter（ ~!=~ ~!~~ ），转化为等于的形式，这样因为等于形式对应的时间线往往会少于取反的效果，最后在合并时，减去这些取反的时间线即可。可参考：[[https://github.com/prometheus-junkyard/tsdb/pull/572][Be smarter in how we look at matchers. #572]]
- 不同 label 的时间线合并时，利用了时间线有序的特点，采用类似 mergesort 的方式来[[https://github.com/prometheus-junkyard/tsdb/pull/616][惰性合并]]，大致过程如下：
  #+begin_src go
...
* 参考
- [[https://my.oschina.net/alchemystar/blog/4985328][Prometheus时序数据库-数据的查询]]
- [[https://my.oschina.net/alchemystar/blog/4965684][Prometheus时序数据库-磁盘中的存储结构]]
- [[https://ganeshvernekar.com/blog/prometheus-tsdb-the-head-block/][Prometheus TSDB (Part 1): The Head Block]]
- [[https://ganeshvernekar.com/blog/prometheus-tsdb-persistent-block-and-its-index/][Prometheus TSDB (Part 4): Persistent Block and its Index]]
- [[https://ganeshvernekar.com/blog/prometheus-tsdb-queries/][Prometheus TSDB (Part 5): Queries]]
- [[https://www.alibabacloud.com/blog/prometheus-the-unicorn-in-metrics_595168][Prometheus: The Unicorn in Metrics]]
