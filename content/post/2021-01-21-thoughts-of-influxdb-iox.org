...
就在写这篇文章的同时，HN 上又出现与这个的话题相关的热贴 [[https://news.ycombinator.com/item?id=25865094][AWS announces forks of Elasticsearch and Kibana]]，只能说开源路漫漫其修远兮，希望 InfluxDB 能成功。
...
因为对于内存性的数据格式而言，压缩会让 CPU 更加负重不堪，由此专门为内存设计的、与语言无关的、列存格式 Arrow 应运而生，而且可以充分利用硬件的特性，比如 SIMD。
...
对 IOx 而言，磁盘上采用 Parquet，内存中使用 Arrow 是个显而易见的选择。这里回答了是什么的问题，但是没有回答为什么要选择列存，下面就来分析这个问题。
...
我大概看了下 DataFusion 的实现，质量非常高，核心开发者 [[https://github.com/andygrove][Andy Grove]] 在查询引擎方面具有[[https://www.andygrove.io/about/][十多年]]的功底，整个项目的设计也非常漂亮，从 LogicalPlan 到 PhysicalPlan 也支持不少优化，在它基础之上来构建自己的引擎是在合适不过了，后面我也会尝试下，看看是否有机会在 [[https://ruby-china.org/topics/40817][CeresDB]] 中引入 DataFusion，对 OLAP 的支持也是 CeresDB 后面的发力点。
