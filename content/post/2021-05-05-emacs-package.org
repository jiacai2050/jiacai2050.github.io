...
#+begin_quote
包是[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Simple-Packages.html][一个]]或[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Multi_002dfile-Packages.html][多个]] ELisp 文件的集合，Emacs 在 =load-path= 指定的文件夹中进行搜索。
#+end_quote
...
Autoload 函数可以声明函数或宏，在真正时使用再去加载其对应的文件。
   #+begin_src lisp
...
这里需要注意，为了让 Emacs 识别到 my-hello 函数，需要去加载 hello-world-autoloads.el 文件，对于通过 package.el 管理的包，package.el 在下载该包时，会进行下面的操作：
1. 解析依赖，递归下载
...
(provide 'hello-world)
#+end_src
上述代码即生成了一个 feature，名为 hello-world，由于与文件同名，可以在使用 my-hello 前 =(require 'hello-world)= ，这样也会去加载 hello-world.el 文件。
...
使用 submodule 的话，只能下载包本身，上述三步都需要自己做，我目前使用 [[https://github.com/jwiegley/use-package][use-package]] 来配置包，下面通过一个示例来介绍其用法：
#+begin_src lisp
...
#+end_src
可以看到，use-package 宏的使用非常简明扼要，而且把包的各种配置都统一起来了。可以使用 macroexpand-1 展开宏，看看背后的逻辑：
#+begin_src lisp
(progn
  (eval-and-compile
...
#+end_src
use-package 解决了配置的问题，但对于包的依赖，只能一个个下载（具体见包的 =Package-Requires= 声明）：
#+begin_src lisp
;; lsp-mode deps
(use-package spinner
  :defer t)
(use-package lv
  :defer t)
#+end_src
use-package 会自动利用 package.el 去下载这些依赖。我这里的做法是折中的，对于一些轻量的包，没必要用 submodule 管理。读者可能会觉得这种手动管理依赖的方式会比较繁琐，但是实际上不同包的依赖很有可能是相同的，比如：[[https://github.com/magnars/dash.el][dash.el]]、[[https://github.com/magnars/s.el][s.el]]、[[https://github.com/rejeep/f.el][f.el]] 等比较基础的包，所以实际需要手动管理的依赖不是很多。
