#+TITLE: 使用 Emacs 阅读邮件与 Feed 信息流
#+DATE: 2021-03-05T23:27:30+08:00
#+TAGS[]: emacs
#+CATEGORIES[]: 效率工具

对于程序员来说，除了编写代码外，阅读邮件与技术文章是耗时比较久的事情，更重要的一点是，这两个的信息输入量都比较大，会涉及到做笔记、TODO 等，如果没有 Emacs，这些事情都是分散在不同应用解决的，比如使用 ThunderBird 作为邮件客户端，Evernote 作为笔记软件。

这些软件或多或少会有不满足需求的地方，用户往往需要使用多个功能类似的软件才能满足需求。邮件客户端我使用过 Foxmail/ThunderBird/Apple Mail ，使用的笔记软件更是不胜枚举，这种十分不优雅的做法终于在遇到 Emacs 之后划上了一个句号，有什么新需求几行 ELisp 就可以搞定，不再有迁移成本。

这篇文章就给读者分享下使用方式与心得。在介绍具体配置前，先展示下效果（主题使用 [[https://github.com/greduan/emacs-theme-gruvbox][gruvbox]]）：

#+CAPTION: mu4e 主界面
[[https://img.alicdn.com/imgextra/i1/581166664/O1CN015qSgGU1z6A2vfHthv_!!581166664.png]]
...
# 过滤掉一些目录，不进行同步
folderfilter = lambda f: 'kK5O9l9SaGM' not in f and '&UXZO1mWHTvZZOQ-' != f and 'liujiacai' not in f
...
#+begin_example
# cat ~/.authinfo
machine imap.exmail.qq.com login hello@liujiacai.net password 123456 port 993
machine smtp.exmail.qq.com login hello@liujiacai.net password 123456 port 465
#+end_example
...
Folderlist:
 ...
 &UXZO1mWHTvZZOQ-/tb@liujiacai.net -> =tb@liujiacai.net (disabled)
 Deleted Messages
 Drafts
 INBOX
 Junk
...
** mu
邮件同步到本地后，在使用前需要使用 mu 建索引，两条命令即可：
#+begin_src bash
# 只需要执行一次 init，index 在每次收取邮件后都需要执行，mu4e 可以配置自动执行
...
- 使用 =mu4e-contexts= 配置多账户
- =mu4e-update-interval= 配置自动同步邮件与建索引间隔
- =mu4e-get-mail-command= 配置同步邮件的命令
- 为 =mu4e-view-actions= 增加 =mu4e-action-view-in-browser= ，这样在阅读邮件时，按 =a b= 在浏览器打开邮件，这对那些只有 html 的邮件来说比较重要
...
- =C-c C-a= 添加附件
* 信息流 Feed
阅读信息流使用的是 [[https://github.com/skeeto/elfeed][elfeed]]，它的配置相比邮件就简单多了，虽然 elfeed 可以不依赖外部软件执行，但是其作者还是推荐使用 curl 而非 ELisp 中的 url-retrieve，一方面是速度更快，另一方面是更稳定。除了 elfeed，还推荐安装下面两个插件：
...
- 为了方便记录重要文章，定义 =my/elfeed-toggle-star= 函数来实现「标星」效果，这样后面可以直接使用 =+starred= 来搜索
...
* 笔记
通过上面的配置，已经可以很好地在 Emacs 中阅读邮件与文章，还剩下如何记笔记没有介绍，对于 Emacser 来说，org-mode 是笔记的第一首选，而且 mu4e/elfeed 都支持 org-capture，直接在当前阅读的邮件或文章中执行 capture 就可以了。

* 总结
通过把邮件与信息流整合进 Emacs，可以充分进行沉浸式阅读，不再需要什么额外的软件，所有操作都在 Emacs 内完成，这是之前从未有过的体验。这可能也是 Emacs 自由、开放的魅力所在。
